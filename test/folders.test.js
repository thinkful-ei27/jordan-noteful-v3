    ...
    it('should update folder with new data', function() {
      ...
      return Folder
        ...
        .then(folder => {
          ...
          expect(folder.content).to.equal(updateFolder.content);
          expect(new Date(folder.createdAt)).to.eql(updateFolder.createdAt);
          expect(new Date(folder.updatedAt)).to.greaterThan(updateFolder.updatedAt);
        });
    });
  });
